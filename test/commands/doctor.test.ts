...
beforeEach(() => {
  state.dir = mkdtempSync(join(tmpdir(), 'redreplier-doctor-'));
...
describe('doctor', () => {
  it('passes every check on a healthy machine and leaves the exit code alone', async () => {
    ...
    expect(checkFor('api').detail).toContain('https://ai.redreplier.com/ai-app/api/v1');
  ...
  it('redacts the token it reports', async () => {
    ...
    expect(stdout).not.toContain('redreplier_ab12cd34');
    expect(checkFor('token').detail).toContain('redreplier_ab12…');
  ...
  it('fails and sets exit 1 when the API cannot be reached', async () => {
    stubFetch({
      ping: () => {
        throw new Error('getaddrinfo ENOTFOUND ai.redreplier.com');
      },
    });
  ...
  it('fails when the API rejects the token', async () => {
    ...
    expect(checkFor('api')).toMatchObject({ status: 'fail', fix: 'redreplier login' });
    expect(process.exitCode).toBe(1);
  });
  ...
  it('warns without failing when a newer release is published', async () => {
    ...
    expect(checkFor('cli')).toMatchObject({ status: 'warn', fix: 'npm i -g @redreplier/cli' });
  ...
  it('fails when no token resolves and skips the authenticated ping', async () => {
    ...
    expect(checkFor('token')).toMatchObject({ status: 'fail', fix: 'redreplier login' });
