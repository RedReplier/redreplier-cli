...
if (isBelow(process.versions.node, NODE_FLOOR)) {
  process.stderr.write(
    `redreplier needs Node ${NODE_FLOOR} or newer, this is Node ${process.versions.node}. ` +
...
const MODULES: Array<{ names: string[]; load: () => Promise<Registrar> }> = [
  ...
  {
    names: ['keyword', 'keywords', 'kw'],
    load: async () => (await import('./commands/keyword.js')).registerKeywordCommands,
  },
  {
    names: ['mention', 'mentions'],
    load: async () => (await import('./commands/mention.js')).registerMentionCommands,
  },
  {
    names: ['alerts', 'alert'],
    load: async () => (await import('./commands/alert.js')).registerAlertCommands,
  },
  {
    names: ['mean-marketer', 'mm'],
    load: async () => (await import('./commands/alert.js')).registerMeanMarketerCommands,
  },
];
...
function wantsEverything(argv: string[], token: string | undefined): boolean {
  ...
  if (rest.includes('-h') || rest.includes('--help') || rest.includes(HELP_ALL_FLAG)) return true;
...
program.addOption(new Option(HELP_ALL_FLAG, 'show help including the experimental commands'));
...
if (process.argv.includes(HELP_ALL_FLAG)) {
  program.configureHelp({ visibleCommands: (command) => [...command.commands] });
