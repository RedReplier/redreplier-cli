...
describe("detectOutputMode", () => {
  ...
  it("is machine with <PRODUCT>_JSON=1", () => {
    expect(detectOutputMode({ isTTY: true, envPrefix: "REDREPLIER", env: { REDREPLIER_JSON: "1" } })).toBe(
      "machine",
    );
  });
  ...
  it("stays human with <PRODUCT>_FORCE_TTY=1 while piped", () => {
    expect(
      detectOutputMode({ isTTY: false, envPrefix: "REDREPLIER", env: { REDREPLIER_FORCE_TTY: "1" } }),
    ).toBe("human");
  });

  it("lets --json win over FORCE_TTY", () => {
    expect(
      detectOutputMode({ isTTY: false, json: true, envPrefix: "REDREPLIER", env: { REDREPLIER_FORCE_TTY: "1" } }),
    ).toBe("machine");
  });
...
describe("human mode", () => {
  ...
  it("writes success to stdout and hints and warnings to stderr", () => {
    ...
    hint("show it: redreplier mention show mention_1");
    warn("rate limit: 14 of 600 left");
    expect(out.join("")).toBe("✓ Post created\n");
    expect(err.join("")).toBe("show it: redreplier mention show mention_1\n⚠ rate limit: 14 of 600 left\n");
  });
...
describe("printError", () => {
  it("prints the message and hint to stderr in human mode", () => {
    const code = printError(
      new ApiError({ status: 404, body: { message: "Post not found" }, hint: "list mentions:  redreplier mention list" }),
      "post.get",
    );
    expect(code).toBe(ExitCode.NOT_FOUND);
    expect(out).toEqual([]);
    expect(err.join("")).toBe("✗ Post not found\n\n  list mentions:  redreplier mention list\n");
  });
