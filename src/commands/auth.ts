...
const PLAN_NOTE = 'API tokens need a plan with API access, so the token page errors on the free plan.';

function domainsOf(websites: readonly Website[]): string[] {
  return websites.map((website) => website.domain).sort();
}

function activeKeywordsOf(websites: readonly Website[]): number {
...
export function assertTokenShape(token: string): void {
  ...
  throw new CliError(
    seen
      ...
      : `That does not look like a ${PRODUCT.displayName} API token, which starts with ${TOKEN_PREFIX}.`,
    {
      exitCode: ExitCode.AUTH,
...
export function registerAuthCommands(program: Command): void {
  program
    ...
    .action(async (options: { tokenStdin?: boolean; name?: string }) => {
      ...
      const keywords = activeKeywordsOf(websites);
      ...
      printKeyValues([
        ...
        ['api', apiUrl],
        ['sites', `${websites.length} monitored`],
        ...(domains.length > 0 ? [['domains', domains.join(', ')] as [string, string]] : []),
  ...
  program
    ...
    .action(async () => {
      ...
      printKeyValues([
        ...
        ['keywords', `${keywords} active`],
        ...(typeof profile.defaults.defaultSite === 'string'
          ? [['default site', `${profile.defaults.defaultSite}  (from config.json)`] as [string, string]]
          : []),
      ...
      hint('There is no /me endpoint, so this is read back from your monitored sites.');
    });
}
