...
const profile: ResolvedProfile = {
  name: 'default',
  token: 'redreplier_ab12cd34',
  tokenSource: 'credentials',
  apiUrl: 'https://ai.example.test/ai-app/api/v1',
  defaults: {},
};
...
describe('buildUrl', () => {
  it('repeats the key for array params and drops null and undefined', () => {
    const url = buildUrl(profile.apiUrl, '/mentions', {
    ...
    expect(url).toBe(
      'https://ai.example.test/ai-app/api/v1/mentions?statuses=NEW&statuses=APPROVED&limit=20&includeLowRelevance=false',
    );
  });
});
...
describe('request', () => {
  ...
  it('sends the auth, accept and user-agent headers', async () => {
    ...
    expect(init.headers.authorization).toBe('Bearer redreplier_ab12cd34');
    expect(init.headers.accept).toBe('application/json');
    expect(init.headers['user-agent']).toMatch(/^redreplier-cli\//);
  ...
  it('serialises a JSON body for writes', async () => {
    ...
    await request({ method: 'POST', path: '/mentions', body: { content: 'hello' } });
    const [, init] = fetchMock.mock.calls[0];
  ...
  it('honours Retry-After on 429 and then succeeds', async () => {
    ...
    const result = await request<{ data: string[] }>({ method: 'GET', path: '/mentions' });
  ...
  it('does not retry a 429 on a write', async () => {
    ...
    await expect(request({ method: 'POST', path: '/mentions', body: {} })).rejects.toMatchObject({ status: 429 });
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it('retries 503 up to three attempts and then throws', async () => {
    fetchMock.mockImplementation(async () => new Response('', { status: 503 }));
    await expect(request({ method: 'GET', path: '/mentions' })).rejects.toMatchObject({ status: 503 });
  ...
  it('retries network failures and surfaces exit code 8', async () => {
    ...
    await expect(request({ method: 'GET', path: '/mentions' })).rejects.toMatchObject({ exitCode: 8 });
    expect(fetchMock).toHaveBeenCalledTimes(3);
  });
  ...
  it('flattens a NestJS validation message array', async () => {
    ...
    await expect(request({ method: 'POST', path: '/mentions', body: {} })).rejects.toMatchObject({
      status: 400,
      code: 'bad_request',
  ...
  it('returns undefined for 204 responses', async () => {
    ...
    await expect(request({ method: 'DELETE', path: '/mentions/mention_1' })).resolves.toBeUndefined();
  });
});
